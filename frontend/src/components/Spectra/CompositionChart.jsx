...
import { Upload, Activity, Loader2, Info, Target, Zap } from 'lucide-react';
...
const CompositionChart = () => {
  ...
  // Rule 2: Create state variable for classification result
  ...
  const getMineralBreakdown = (type) => {
    const mappings = {
      ...
      'M-type': [{ name: 'Iron-Nickel', value: 60 }, { name: 'Olivine', value: 20 }, { name: 'Pyroxene', value: 15 }, { name: 'Other', value: 5 }],
      'V-type': [{ name: 'Basalt', value: 70 }, { name: 'Pyroxene', value: 20 }, { name: 'Olivine', value: 8 }, { name: 'Other', value: 2 }],
    };
    return mappings[type] || mappings['S-type'];
  };
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      // Rule 1: Log the full response to console
      console.log('API Response:', response.data);
      
      // Rule 3: Update BOTH state variables
      setClassificationResult(response.data);
      setChartMinerals(getMineralBreakdown(response.data.type));
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-10 font-outfit text-white min-h-screen">
      ...
      {/* Rule 7: Conditional Render of Result Card with exact class and structure */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-5 duration-500">
          <div className="result-card glass-panel p-10 rounded-[40px] border border-white/10 flex flex-col justify-center relative overflow-hidden">
            ...
            <div className="space-y-8 relative z-10">
              <div>
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.4em] mb-4 block underline underline-offset-8 decoration-blue-500/30">Taxonomic Identification</span>
                <h3 className="text-7xl font-black italic text-white tracking-tighter">Type: {classificationResult.type}</h3>
              </div>
              <div className="grid grid-cols-2 gap-8 pt-4">
                <div className="space-y-1">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Confidence</p>
                  <p className="text-4xl font-black text-indigo-400">{classificationResult.confidence}%</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Mining Potential</p>
                  <div className="flex items-center gap-3">
                    <Target className="w-6 h-6 text-green-400" />
                    <p className="text-4xl font-black text-white">{classificationResult.mining_potential}</p>
                  </div>
                </div>
              </div>
          ...
          <div className="glass-panel p-10 rounded-[40px] border border-white/10 flex flex-col items-center shadow-2xl min-h-[450px] relative overflow-hidden">
            ...
            <div className="w-full h-[300px] relative flex items-center justify-center z-10">
              ...
              <Chart 
                key={classificationResult?.type || 'default'} 
                data={{
              ...
              <div className="absolute flex flex-col items-center justify-center pointer-events-none translate-y-[-10px]">
                ...
                <span className="text-4xl font-black italic">{classificationResult.type?.split('-')[0] || 'N/A'}</span>
              </div>
            </div>
          </div>
        </div>
